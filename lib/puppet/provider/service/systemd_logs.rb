...
Puppet::Type.type(:service).provide :systemd_logs, :parent => :base do
  desc "Manages `systemd` services using `systemctl`, with extra logging!.
  ...
  def enabled?
    ...
    unless @resource[:enable].nil?
      return :mask if (@resource[:enable].to_sym == :mask) && (output == 'masked')
    end
  ...
  def prepare_error_message(name, action, exception)
    ...
    journalctl_output = execute("journalctl -n 50 --since '5 minutes ago' -u #{name} --no-pager")
    error_return << "journalctl log for #{name}: #{journalctl_output}"
    raise Puppet::Error, error_return, exception.backtrace
  end
end
